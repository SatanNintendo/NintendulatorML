...
INT_PTR CALLBACK        DllSelect (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
        ...
        {
        case WM_INITDIALOG:
                SetWindowText(hDlg, Lang::GetString(LANG_DLG_DLL_TITLE));
                ...
                SetDlgItemText(hDlg, IDCANCEL, Lang::GetString(LANG_DLG_CANCEL));
                {
                        ...
                        while (hChild) {
                                TCHAR txt[256] = {0};
                                GetWindowText(hChild, txt, 256);
                                if (_tcscmp(txt, _T("This ROM is supported by multiple mapper DLLs.\nPlease select the one you wish to use.")) == 0)
                                        { SetWindowText(hChild, Lang::GetString(LANG_DLG_DLL_TEXT)); break; }

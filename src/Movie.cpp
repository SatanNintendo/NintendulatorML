...
INT_PTR CALLBACK        MoviePlayProc (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_COMMAND:
                ...
                case IDC_MOVIE_PLAY_BROWSE: {
                        ...
                        ofn.hwndOwner = hDlg;
                        ofn.hInstance = hInst;
                        TCHAR MovPlayFilter[256];
                        TCHAR *pMPF = MovPlayFilter;
                        _tcscpy(pMPF, Lang::GetString(LANG_FILTER_MOVIE)); pMPF += _tcslen(pMPF) + 1;
                        _tcscpy(pMPF, _T("*.NMV")); pMPF += _tcslen(pMPF) + 1;
                        *pMPF = 0;
                        ofn.lpstrFilter = MovPlayFilter;
                        ...
                        if (tvmode & 0x80)
                                SetDlgItemText(hDlg, IDC_MOVIE_PLAY_TVMODE, Lang::GetString(LANG_MOVIE_TV_PAL));
                        else    SetDlgItemText(hDlg, IDC_MOVIE_PLAY_TVMODE, Lang::GetString(LANG_MOVIE_TV_NTSC));
                        ...
                        _stprintf(str, Lang::GetString(LANG_MOVIE_LENGTH_FMT), Len / 3600, (Len % 3600) / 60, Len % 60);
...
INT_PTR CALLBACK        MovieRecordProc (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_COMMAND:
                ...
                case IDC_MOVIE_RECORD_BROWSE: {
                        ...
                        ofn.hwndOwner = hDlg;
                        ofn.hInstance = hInst;
                        TCHAR MovRecFilter[256];
                        TCHAR *pMRF = MovRecFilter;
                        _tcscpy(pMRF, Lang::GetString(LANG_FILTER_MOVIE)); pMRF += _tcslen(pMRF) + 1;
                        _tcscpy(pMRF, _T("*.NMV")); pMRF += _tcslen(pMRF) + 1;
                        *pMRF = 0;
                        ofn.lpstrFilter = MovRecFilter;
                        ofn.lpstrCustomFilter = NULL;
                        ofn.nFilterIndex = 1;
                        ofn.lpstrFile = filename;
                        ofn.nMaxFile = MAX_PATH;
...
unsigned char   LoadInput (void)
{
        ...
        if (Pos >= Len)
        {
                ...
                else    PrintTitlebar(Lang::GetString(LANG_MOVIE_UNEXPECTED_EOF));
                EndMovie();
        }
        if (Controllers::Port1->MovLen)

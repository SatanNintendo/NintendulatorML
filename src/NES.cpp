...
void    SaveSettings (void)
{
        ...
        Theme::SaveSettings(SettingsBase);
...
void    LoadSettings (void)
{
        ...
        Theme::LoadSettings(SettingsBase);
...
void    RelocateSaveData_Mydocs (void)
{
        ...
        {
                TCHAR str[MAX_PATH * 2 + 256];
                _stprintf(str, Lang::GetString(LANG_ERR_NES_RELOCATE), DataPath, oldPath);
                MessageBox(NULL, str, Lang::GetString(LANG_DLG_NINTENDULATOR), MB_OK | MB_ICONERROR);
                BrowseFolder(oldPath);
        }
}

// Re-apply menu enable/disable states based on current ROMLoaded state.
// This is needed because Lang::UpdateMenu() calls ModifyMenu(MF_STRING)
// which resets MF_GRAYED on all menu items, re-enabling disabled items.
void    SyncMenuStates (void)
{
        if (ROMLoaded)
        {
                // Game menu - enable only if mapper has config
                if (MI && MI->Config)
                        EnableMenuItem(hMenu, ID_GAME, MF_ENABLED);
                else
                        EnableMenuItem(hMenu, ID_GAME, MF_GRAYED);
                ...
                // Misc - stop items are always disabled unless actively capturing/playing
                EnableMenuItem(hMenu, ID_MISC_STOPAVICAPTURE, MF_GRAYED);
                EnableMenuItem(hMenu, ID_MISC_STOPMOVIE, MF_GRAYED);
        }
        else
        {
                // No ROM loaded - disable ROM-dependent items
                EnableMenuItem(hMenu, ID_GAME, MF_GRAYED);
                EnableMenuItem(hMenu, ID_FILE_CLOSE, MF_GRAYED);
                ...
                // Game Genie stays enabled without ROM (can be toggled)
                EnableMenuItem(hMenu, ID_CPU_GAMEGENIE, MF_ENABLED);

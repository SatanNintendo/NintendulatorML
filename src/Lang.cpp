...
void Lang::UpdateMenu(HMENU hMenu)
{
    ...
    // Топ-уровневые пункты меню (по позиции)
    HMENU hSub;

    // File
    hSub = GetSubMenu(hMenu, 0);
    if (hSub) ModifyMenu(hMenu, 0, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_FILE));

    // NES
    hSub = GetSubMenu(hMenu, 1);
    if (hSub) ModifyMenu(hMenu, 1, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_NES));

    // Settings
    hSub = GetSubMenu(hMenu, 2);
    if (hSub) ModifyMenu(hMenu, 2, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_SETTINGS));

    // Movie
    hSub = GetSubMenu(hMenu, 3);
    if (hSub) ModifyMenu(hMenu, 3, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_MOVIE));

    // Debug
    hSub = GetSubMenu(hMenu, 4);
    if (hSub) ModifyMenu(hMenu, 4, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_DEBUG));

    // Language (позиция может быть 5)
    hSub = GetSubMenu(hMenu, 5);
    if (hSub) ModifyMenu(hMenu, 5, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_LANGUAGE));

    // Help
    hSub = GetSubMenu(hMenu, 6);
    if (hSub) ModifyMenu(hMenu, 6, MF_BYPOSITION | MF_STRING, (UINT_PTR)hSub, GetString(LANG_MENU_HELP));
